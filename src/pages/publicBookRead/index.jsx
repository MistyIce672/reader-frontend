...
import CommonWordsPopup from "../read/components/CommonWordsPopup";

const PublicBookRead = () => {
  ...
  useEffect(() => {
    const fetchPageContent = async () => {
      try {
        setLoading(true);
        const content = await publicBookService.getPage(bookId, page);
  ...
  const handlePreviousPage = () => {
    if (page > 1) {
      navigate(`/public-books/${bookId}/read/${parseInt(page) - 1}`);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      {/* Public Book Badge */}
      <div className="bg-green-50 border border-green-200 rounded-md p-3 mt-4 text-center">
        <span className="text-green-800 text-sm font-medium">
          Public Book — 5x cheaper per page
        </span>
      </div>
      {/* Common Words Popup */}
